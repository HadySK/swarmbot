...
#values that is used in estimating the field of view
...
status = DETECT_PEER #global variable used to control when to combute the robots seen by the robot
...
def control():
	global Pose_rob, Pose_rob2, Pose_rob3, status, rob_seen
	...
	status = STOPDETECT_PEER


def main():
	global sub_rob, pub_rob, status, rob_seen
