...
#values that is used in 'status' variable
DETECT_LEADER = 1
STOPDETECT_LEADER = 0
...
pub_goal = None #Publisher to publish the leader goal after determining the leader

goalpose = PoseStamped() #stores in it the leader goal
Dist_rob1 = None #stores the distance between the robot and the leader goal
Dist_rob2 = None #stores the distance between the robot and the leader goal
Dist_rob3 = None #stores the distance between the robot and the leader goal
Distance = [Dist_rob1 , Dist_rob2 , Dist_rob3] #list to store the distance between the robots and the leader goal
Near_ID = 0 #Nearest Robot ID to Goal
...
status = DETECT_LEADER #controls when to combute which robot is leader and which is fol1 and fol2
...
def distance(msg):
	global Pose_rob, Pose_rob2, Pose_rob3, Near_ID, Distance, goalpose
	goalpose = msg
	...
	for i in range(0,3):
		if(Distance[i] < Distance[Near_ID]):
			Near_ID = i
	print(Near_ID)
	if(status == DETECT_LEADER):
		control()

#call this function when status = DETECT_LEADER to combute the status of the robot
def control():
	global robs_seen, Pose_rob, Pose_rob2, Pose_rob3, leader, follower1, follower2, status, Near_ID
	...
	status = STOPDETECT_LEADER

def main():
	global sub_rob1, sub_rob2, sub_rob3, sub_rob_pos, sub_rob2_pos, sub_rob3_pos, pub_led, pub_fol1, pub_fol2, leader, follower1, follower2, pub_goal, goalpose
