...
pub_isNarrow = rospy.Publisher('/isNarrow',Bool,queue_size=1000) #Publisher to publish on a topic if there is a narrow path
...
def isNarrow():
	...
	x = l+l2
	if( x < 1.0 and x > 0.7):
		pub_isNarrow.publish(True)
	else :
		pub_isNarrow.publish(False)
